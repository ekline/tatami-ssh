...
## 0. Implementation status (added 2026-09-20, after the sections below were written; updated for round 4)

The Cargo workspace now exists with all seven packages (see `architecture.md`, decision W-01 supersedes the "create lazily" advice in §2). Rounds 1–4 are done and tested:
...
| Fuzzing (round 3) | libFuzzer targets with independent oracles over every implemented surface; one production finding fixed (tombstone capacity at 0). Round 4 added targets for negotiation, GCM packets, key blobs, KEX codecs and the handshake; the remote ASan CI run after the W-34 install fix is still pending. | `fuzz/`, `fuzzing.md` |
| **TCP verified transport diagnostic (round 4)** | Implemented: `tatami-client handshake` completes `curve25519-sha256`, verifies the `ssh-ed25519` signature over `H`, applies the operator pin, exchanges `NEWKEYS`, protects packets with `aes128-gcm@openssh.com` per direction, negotiates strict KEX (both client spellings offered; OpenSSH 10.2 matches the pre-standard one), receives `EXT_INFO` (`server-sig-algs` recorded), requests and receives `ssh-userauth`, sends a protected `DISCONNECT`. Never sends `USERAUTH_REQUEST`; refuses rekey (`RekeyNotSupported`). Client role only. Verified against `OpenSSH_10.2p1`: default completes; wrong pin → `HostNotTrusted` with no `NEWKEYS` sent; profile-restricted sshd completes; `Ciphers=aes256-ctr` → `NegotiationFailed(NoCommonCipher)` (W-29, W-30, W-33). | `tatami-wire::{kex,ext_info,algorithms,primitives::Mpint}`, `tatami-tcp::{negotiate,transcript,gcm,handshake,io::handshake}`, `tatami::client::handshake`; evidence `tatami-tcp/tests/openssh_handshake.rs`, `tatami/tests/handshake_cli.rs`, `scripts/openssh-fixture.sh` |
| **Keys and host trust (round 4)** | Implemented: public-key/signature blob codecs with key-format and signature-algorithm names kept distinct (RFC 8332 motivation), Ed25519 verification (`verify_strict`, RFC 8032 §7.1 vectors), `SHA256:` fingerprint of the complete blob (equals `ssh-keygen -lf`), `HostTrustPolicy` contract with `PinnedSha256` (W-32) and `NoTrustPolicy`. Signature validity and trust are separate reported facts; both gate `NEWKEYS`. No RSA/ECDSA/certificates, no signing, no `known_hosts`. | `tatami-keys::{blob,ed25519,fingerprint,trust}` |
| **Connection fix (round 4)** | `handle_open_confirmation` rejects a peer `sender_channel` already live (`Violation::DuplicatePeerNumber`) before any state change; late confirmations keep their tombstone. | `tatami-connection::opening` |
| **QUIC diagnostic handshake observer (round 4)** | Implemented as observer (a) of `quic-observer-readiness.md`: `quinn-proto` 0.11.18 + `rustls` 0.23.45/`ring`, sans-I/O cores, blocking UDP loop; reports source address and validation state (`--require-validation` → Retry, `validation_method: retry_token`), offered vs negotiated ALPN and SNI, outcome, exporter availability; 0-RTT/resumption disabled explicitly; streams/DATAGRAM refused; **no SSH byte sent** (asserted over every datagram). ALPN `tatami-diag/0` experimental, unregistered; loopback only; Tatami on both ends (W-31). Corrections learned are in `quic-observer-readiness.md` §4. | `tatami-quic::diag::{server,client,tls,identity,inmem,udp}`, `tatami::quic_diag`, `tatami-quic-server`, `tatami-quic-client`; evidence `tatami-quic/tests/{inmem_handshake,loopback}.rs`, `tatami/tests/quic_cli.rs` |
| **Raw public keys in TLS (round 4)** | RFC 7250 handshake demonstrated over QUIC through quinn-proto/rustls: server `AlwaysResolvesServerRawPublicKeys`, client verifier pinning the SPKI SHA-256 with rustls verifying `CertificateVerify`; `peer_identity()` is the 44-byte Ed25519 SPKI (RFC 8410 prefix); wrong pin fails; no silent downgrade either way. The certificate, SPKI and `ssh-ed25519` blob fingerprints of one key all differ. Not on the CLI. **SSH identity mapping still open (P-06).** | `tatami-quic::diag::{tls,identity}`; evidence `tatami-quic/tests/rpk.rs` |
| **TLS exporter (round 4)** | Availability shown: `export_keying_material` fails before completion, agrees at both ends, differs by label/context/length/connection; experimental label, output discarded. **Binding construction not defined (P-04, AQ-003).** | evidence `tatami-quic/tests/exporter.rs` |
| SFTP / file transfer | Direction recorded (W-25); nothing implemented | — |
| Userauth, rekeying, channels/data/window/EOF/close, PTY/exec/forwarding | Not started; next TCP milestone in this order: rekey, `publickey` userauth, `session`/`exec` channel, forwarding | — |
| QUIC record framing, control stream/bootstrap, identification placement, association, session binding (AQ-015, AQ-018, AQ-020–AQ-023, AQ-026, P-04) | Not started; no wire choice made; observer (b) blocked on these | — |
...
**Protocol notes from the handshake (round 4):** AEAD names appear only in the cipher lists and the MAC lists are ignored when an AEAD is selected (draft-miller-sshm-aes-gcm-01 §2, OpenSSH `PROTOCOL` §1.6); Tatami's MAC lists carry `hmac-sha2-256` only to satisfy RFC 4253 §7.1 syntax, and this is recorded as a gap (W-30). Strict KEX must be matched within one spelling (pre-standard with pre-standard); OpenSSH 10.2 offers only `kex-strict-s-v00@openssh.com`. OpenSSH sends `EXT_INFO` immediately after `NEWKEYS`; the client accepts it there whether or not it offered `ext-info-c`. On the QUIC side: an ALPN mismatch fails inside `Endpoint::accept` before any connection exists; Version Negotiation is not an observable event in quinn-proto 0.11; address validation, Retry and TLS identity are three separate facts; quinn-proto's convenience constructors enable early data, so rustls configs are built directly. None of this settles a wire-format question.

Item 1 of "Next concrete work" in §5 is complete. Validation rows "Message codecs" and "Opening state" in §5 have executable evidence in package tests; "Interoperability" now has automated Tatami-client ↔ OpenSSH-sshd tests for the transport handshake only (`tatami-tcp/tests/openssh_handshake.rs`), not full-session interoperability and not the server role.
...
| P-04 / D-006, AQ-003 | TLS 1.3/QUIC supplies key establishment and protection; an exporter-derived SSH/QUIC binding supplies the identifier needed by userauth. | Label, context, output length, transcript inputs and authentication proof. *Availability* of the RFC 8446 §7.5 exporter through quinn-proto/rustls is shown (round 4, `tatami-quic/tests/exporter.rs`: unavailable before completion, equal at both ends, separated by label/context/length/connection); the **construction is still open**. |
...
| P-06 / trust model | TLS raw public keys are the preferred candidate for familiar host-key trust. | Implementation capability is now demonstrated (round 4, `tatami-quic/tests/rpk.rs`: RFC 7250 handshake over QUIC with rustls 0.23, SPKI pinning, proof of possession enforced). **Still open:** the SSH identity mapping — the SPKI DER, the `ssh-ed25519` blob and a certificate of the same Ed25519 key have three different SHA-256 fingerprints, so which one an operator pins and how it maps to `known_hosts` semantics is undecided. |
...
| P-08 / AQ-019 | ALPN value is configurable during experiments; no standardized Tatami identifier is asserted. | An ALPN-free deployment requires a defined authenticated alternative, not merely a known port. Round 4 uses `tatami-diag/0` in the diagnostic handshake only, configurable and required; no registration, no SSH meaning. AQ-019 unchanged. |
...
| AQ-003 and subquestions, AQ-005, AQ-024 | Exporter construction, identity validation and exact userauth signature integration. **Remains open**: round 4 showed only that the exporter is available and behaves as RFC 8446 §7.5 specifies; no label, context or transcript input has been chosen. | Authenticated SSH/QUIC operation. |
